...
from geometry_msgs.msg import PoseStamped, Twist, Pose, Point, Vector3Stamped,TwistStamped
...
class Px4Controller:
    def __init__(self):
        ...
        self.uav_local_pose_sub = rospy.Subscriber("/uav" + str(uav_ID) + "/mavros/local_position/pose", PoseStamped,
                                                   self.uav_local_position_callback)
        self.uav_local_pose_sub = rospy.Subscriber("/uav" + str(uav_ID) + "/mavros/local_position/velocity",
                                                   TwistStamped,
                                                   self.uav_local_velocity_callback)
    ...
    def start(self):
        ...
        while self.arm_state and self.offboard_state and (rospy.is_shutdown() is False):
            if self.local_pose.pose.position.z < 2.2 and self.local_pose.pose.position.z > 1.8:
                ...
                self.TargetVelocity.linear.x = 0.2
                self.TargetVelocity.linear.y = 0.2
                self.TargetVelocity.linear.z = 0.2  # velocity
                self.TargetAcc.vector.x = 0.1
                self.TargetAcc.vector.y = 0.1
                self.TargetAcc.vector.z = 0           # accel
                self.local_target_v_pub.publish(self.TargetVelocity)
                # self.local_target_a_pub.publish(self.TargetAcc)
                data_x.append(self.local_pose.pose.position.x)
                data_y.append(self.local_pose.pose.position.y)
                data_z.append(self.local_pose.pose.position.z)
                data_vx.append(self.local_velocity.linear.x)
                data_vy.append(self.local_velocity.linear.y)
                data_vz.append(self.local_velocity.linear.z)
                k = k+1
                print k
                if k > 500:
                    data_uav = [data_x, data_y, data_z, data_vx, data_vy, data_vz]
                    name = ['x', 'y', 'z', 'vx', 'vy', 'vz']

                    Data = pd.DataFrame(dict(zip(name, data_uav)))
                    Data.to_csv('/home/hwk/program/2021-6-15-1uav_test/v.csv')
